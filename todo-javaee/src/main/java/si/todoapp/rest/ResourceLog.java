...
public class ResourceLog implements ContainerResponseFilter, ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext requestContext, ContainerResponseContext responseContext)
        ...
        StringBuilder stringBuilder = new StringBuilder();
        createResponseMessage(responseContext, stringBuilder);

        log.info("Server responded on request {0} {1} with response: {2}", requestContext.getMethod(), requestContext
            .getUriInfo().getRequestUri().toASCIIString(), stringBuilder.toString());
    }

    private void createRequestMessage(ContainerRequestContext requestContext, StringBuilder stringBuilder) {
    ...
    private void createResponseMessage(ContainerResponseContext responseContext, StringBuilder stringBuilder) {
        stringBuilder.append(NEW_LINE);
